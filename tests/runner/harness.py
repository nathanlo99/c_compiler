...
bril/optimized/mips/interpret golden -- absence, not an empty file, is
how "doesn't get this far" is represented).
"""
...
CATEGORIES = ["optimal", "gaps", "benchmarks", "broken"]
...
@dataclasses.dataclass(frozen=True)
class Case:
    """A single test case: `tests/cases/<category>/<name>/`."""
    category: str
    name: str
    dir: pathlib.Path
...
def golden_paths(case: Case, stage: Stage) -> tuple[pathlib.Path, pathlib.Path]:
    """The (stdout, return-code) golden file paths for `case`'s `stage`."""
    return case.dir / f"{stage.name}.out", case.dir / f"{stage.name}.rc"


def read_golden(case: Case, stage: Stage) -> RunResult | None:
    """The checked-in golden result for `case`'s `stage`, or None if there
    isn't one yet."""
    out_path, rc_path = golden_paths(case, stage)
    if not out_path.exists():
        return None
    text = out_path.read_text(encoding="utf-8")
    rc = int(rc_path.read_text(encoding="utf-8").strip()) if rc_path.exists() else 0
    # We don't distinguish stdout/stderr in the golden file itself -- see
    # RunResult.golden_text -- so store it back into `stdout` for diffing.
    return RunResult(returncode=rc, stdout=text, stderr="")


def write_golden(case: Case, stage: Stage, result: RunResult) -> None:
    """Writes `result` as the new golden output for `case`'s `stage`."""
    out_path, rc_path = golden_paths(case, stage)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(result.golden_text, encoding="utf-8")
    rc_path.write_text(f"{result.returncode}\n", encoding="utf-8")


def clear_golden(case: Case, stage: Stage) -> None:
    """Remove a stage's golden files, e.g. because an earlier stage now
