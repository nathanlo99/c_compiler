...
files under each tests/cases/<category>/<name>/ directory.
...
Usage:
    tests/runner/review_golden.py                    # interactive, everything
    tests/runner/review_golden.py --category gaps
...
def print_diff(outcome: Outcome) -> None:
    ...
    golden (or a note that it's orphaned / that only the return code differs)."""
    ...
    golden = harness.read_golden(outcome.case, outcome.stage)
    old_lines = golden.stdout.splitlines(keepends=True) if golden else []
    new_lines = result.golden_text.splitlines(keepends=True)
    diff = list(difflib.unified_diff(
        old_lines, new_lines,
        fromfile="golden" if golden else "(missing)",
        tofile="live",
    ))
    if diff:
        print("".join(diff), end="" if diff[-1].endswith("\n") else "\n")
    else:
        print(f"(same text; return code differs -- golden="
              f"{golden.returncode if golden else 'n/a'} live={result.returncode})")
