...
`tests/unit/` and diffing the result against what the source should
reduce to. Each repro below is a `tests/unit/<name>/` directory; a
test that's a known repro for one of these items has a `TO_FIX` file in
its directory naming the item, so it's not expected to reduce to the
fully-optimal form documented here yet. Run `tests/run_tests.py` to
check against golden outputs, or `tests/review_golden.py` to accept
new/changed ones.
...
**Repro:** `tests/unit/constant_folding_reassociate/input.c`
(`return a + 1 + b + 1;`) and `tests/unit/constant_folding_negate_add/input.c`
(`return 0 - a + b;`)
**Expected:** `a + 1 + b + 1` → 2 instructions (`add a b` then `add _ 2`),
not 3; `0 - a + b` → 1 instruction (`sub b a`), not 2.
...
both.
**Repro:** `tests/unit/algebra_add_sub_cancel/input.c` (`return a + b - b;`)
...
**Repro:** `tests/unit/algebra_mul_two_rhs/input.c` (`return a * 2;`) and
`tests/unit/algebra_mul_two_lhs/input.c` (`return 2 * a;`)
...
**Repro (memory-triggered):** `tests/unit/vector/input.c`,
`new_and_delete`, `print_all`, `pointer_init`, `print`, `all_tokens`,
`augmented`, `simple2` (all under `tests/unit/`)
**Repro (loop-only, no memory):** `tests/unit/simple_loop/input.c`,
`tests/unit/loop/input.c`
...
**Repro:** `tests/unit/inline_functions/input.c`
...
**Repro:** `tests/unit/memory_leaks/input.c`
...
**Repro:** `tests/unit/mutual_recursion/input.c`
...
BRIL-level call.
**Repro:** `tests/unit/tail_recursion/input.c`
...
Also `tests/unit/deep_recursion/input.c` (`f(f(f(100000)))`, tail-shaped
...
**Repro:** `tests/unit/loop/input.c` — 10 fixed iterations, no
...
**Repro:** `tests/unit/simple/input.c` — `wain(int a, int b)`,
`b` unused.
